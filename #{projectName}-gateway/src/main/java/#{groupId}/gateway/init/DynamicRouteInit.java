...
import #{groupId}.gateway.entity.GatewayRoute;
...
/**
 * 动态路由初始化类
 * 请将/doc/nacos目录下的#{projectName}-dynamic-routes.yaml配置至nacos
 *
 * @author #{version}
 * @since 2.3.8
 */
...
public class DynamicRouteInit {
	...
	public void initRoute() {
		try {
			...
			String content = configService.getConfig(MineConstant.CONFIG_DATA_ID_DYNAMIC_ROUTES, nacosProperties.getGroup(), MineConstant.CONFIG_TIMEOUT_MS);
			log.info("初始化网关路由开始");
			updateRoute(content);
			log.info("初始化网关路由完成");
			//开户监听，实现动态
			configService.addListener(MineConstant.CONFIG_DATA_ID_DYNAMIC_ROUTES, nacosProperties.getGroup(), new Listener() {
				@Override
