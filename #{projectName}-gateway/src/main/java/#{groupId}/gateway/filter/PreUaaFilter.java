...
import #{groupId}.core.common.constant.WebConstant;
import #{groupId}.core.common.util.ResponseUtil;
import #{groupId}.core.common.util.SecurityUtil;
import #{groupId}.core.common.util.StringPool;
import #{groupId}.core.common.util.TokenUtil;
...
public class PreUaaFilter implements GlobalFilter, Ordered {
	...
	private final MineApiProperties mineApiProperties;

	private final RedisService redisService;

	/**
	 * 路径前缀以/#{projectName}开头，如#{projectName}-system
	 */
	public static final String PATH_PREFIX = "/#{projectName}";
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		// 如果未启用网关验证，则跳过
		if (!mineApiProperties.getEnable()) {
	...
	private boolean ignore(String path) {
		return mineApiProperties.getIgnoreUrl().stream()
				.map(url -> url.replace("/**", ""))
	...
	@Override
	public int getOrder() {
		return MineConstant.UAA_FILTER_ORDER;
	}
