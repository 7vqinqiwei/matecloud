...
import #{groupId}.core.common.constant.Oauth2Constant;
import #{groupId}.core.redis.core.RedisService;
import #{groupId}.core.security.userdetails.MineUser;
import #{groupId}.uaa.granter.CaptchaTokenGranter;
import #{groupId}.uaa.granter.SmsCodeTokenGranter;
import #{groupId}.uaa.granter.SocialTokenGranter;
import #{groupId}.uaa.service.impl.ClientDetailsServiceImpl;
import #{groupId}.uaa.service.impl.SingleLoginTokenServices;
...
public class AuthServerConfig extends AuthorizationServerConfigurerAdapter {
	...
	@Value("${#{projectName}.uaa.isSingleLogin:false}")
	...
	public TokenEnhancer tokenEnhancer() {
		return new TokenEnhancer() {
			...
			public OAuth2AccessToken enhance(OAuth2AccessToken oAuth2AccessToken, OAuth2Authentication oAuth2Authentication) {
				...
				// 获取当前登录的用户
				MineUser user = (MineUser) oAuth2Authentication.getUserAuthentication().getPrincipal();

				// 如果用户不为空 则把id放入jwt token中
				if (user != null) {
					additionMessage.put(Oauth2Constant.SYS_USER_ID, String.valueOf(user.getId()));
					additionMessage.put(Oauth2Constant.SYS_USER_NAME, user.getUsername());
					additionMessage.put(Oauth2Constant.SYS_USER_AVATAR, user.getAvatar());
					additionMessage.put(Oauth2Constant.SYS_USER_ROLE_ID, String.valueOf(user.getRoleId()));
					additionMessage.put(Oauth2Constant.SYS_USER_TYPE, user.getType());
					additionMessage.put(Oauth2Constant.SYS_TENANT_ID, user.getTenantId());
				}
