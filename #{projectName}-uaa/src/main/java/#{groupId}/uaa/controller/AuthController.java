package #{groupId}.uaa.controller;
...
import #{groupId}.core.common.util.StringPool;
...
import #{groupId}.system.dto.UserInfo;
import #{groupId}.system.feign.ISysRolePermissionProvider;
import #{groupId}.system.feign.ISysUserProvider;
import #{groupId}.uaa.config.SocialConfig;
import #{groupId}.uaa.enums.LoginType;
import #{groupId}.uaa.service.ValidateService;
...
public class AuthController {
    ...
    @GetMapping("/get/user")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "#{ProjectName}-Auth", required = true, value = "授权类型", paramType = "header")
    })
    ...
    public Result<?> getUser(HttpServletRequest request) {
        ...
        // 存入redis,以用于#{ProjectName}-starter-auth的PreAuthAspect查询权限使用
        redisService.set(Oauth2Constant.SYS_PERMISSION_PREFIX + loginUser.getAccount()
    ...
    public Map<String, String> loginType() {
        ...
        return oauthList.stream().collect(Collectors.toMap(oauth -> oauth.toLowerCase() + "登录", oauth -> "http://localhost:10001/#{projectName}-uaa/auth/login/" + oauth.toLowerCase()));
    }
