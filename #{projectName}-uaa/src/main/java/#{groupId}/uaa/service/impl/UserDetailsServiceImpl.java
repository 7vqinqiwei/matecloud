package #{groupId}.uaa.service.impl;
...
import #{groupId}.core.security.userdetails.MineUserDetailsService;
...
@Slf4j
@Service
public class UserDetailsServiceImpl implements MineUserDetailsService {
	...
	private UserDetails getUserDetails(UserInfo userInfo) {
		...
		return new MineUser(user.getId(), userInfo.getType(), user.getDepartId(), user.getRoleId(), user.getTelephone(), user.getAvatar(),
