package #{groupId}.core.web.listener;
...
public class RequestMappingScanListener implements ApplicationListener<ApplicationReadyEvent> {
	...
	public void onApplicationEvent(@NotNull ApplicationReadyEvent event) {
		...
		try {
			...
			res.put("list",list);
			redisService.hset(MineConstant.CLOUD_API_RESOURCE, microService, res, 18000L);
			redisService.sSetAndTime(MineConstant.CLOUD_SERVICE_RESOURCE, 18000L, microService);
			log.info("资源扫描结果:serviceId=[{}] size=[{}] redis缓存key=[{}]", microService,  list.size(), MineConstant.CLOUD_API_RESOURCE);
		} catch (Exception e) {
